...
const CardDetailSubject = ({ id }) => {
    ...
    return (
        <div>
            {subject.map((detail, index) => (
                detail.subject_id === id && (
                    <div className="max-w p-6 bg-white border border-gray-200 rounded-xl" key={index}>
                        <div className="flex flex-row">
                            <h5 className="mb-2 text-xl font-bold text-[#151C38]">{detail.subject_id}</h5>
                            <div>
                                <h5 className="mb-2 text-xl font-bold text-[#151C38] ml-10">{detail.subject_name_en} ({detail.subject_name_th})</h5>
                                <p className="ml-10 text-sm font-medium text-[#151C38]">{detail.detail}</p>
                            </div>
                        </div>
                        <p className="mt-4 text-sm font-light text-[#151C38]">หน่วยกิต(ทฤษฎี - ปฎิบัติ - ค้นคว้า) {detail.subject_credit}</p>
                    </div>
                )
            ))}
        </div>
